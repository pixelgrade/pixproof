 ...
 */
class PixproofProcessorImpl implements PixproofProcessor {
	...
	/** @var PixproofMeta current data; including submitted data */
	protected $data = null;
		...
		try {
			...
			if ($this->form_was_submitted()) {
				...
				if (empty($errors)) {
					...
					$this->data = pixproof::instance('PixproofMeta', $input);
	...
	protected function load_data_from_database($option_key) {
		...
		$this->data = pixproof::instance('PixproofMeta', $dbconfig);
	}
	...
	protected function validate_input($input) {
		$validator = pixproof::instance('PixproofValidator', $this->meta, $this->fields);
	...
	/**
	 * @return PixproofMeta current data (influenced by user submitted data)
	 */
	function data() {
		if ($this->status === null) {
			$this->run();
