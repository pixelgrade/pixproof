 ...
 */
class PixproofFormImpl extends PixproofHTMLElementImpl implements PixproofForm {
	...
	protected function configure($config = null) {
		...
		$this->autocomplete = pixproof::instance('PixproofMeta', array());

		// setup fields
		$this->fields = pixproof::instance('PixproofMeta', $config['fields']);
		unset($config['fields']);
	...
	function field($fieldname, $fieldconfig = null) {
		...
		return pixproof::instance('PixproofFormField', $fieldconfig)
			->setmeta('form', $this)
	...
	function fieldtemplate($templatepath, $conf = array()) {
		$config = pixproof::instance('PixproofMeta', $conf);
		return $this->fieldtemplate_render($templatepath, $config);
	}

	/**
	 * @param string template path
	 * @param PixproofMeta configuration
	 * @return string
	 */
	protected function fieldtemplate_render($_template_path, PixproofMeta $conf) {
